...
(function (root) {
  ...
  function renderLocations(groups){var gs=groups.slice().sort(function(a,b){return b.value-a.value;}).slice(0,10).map(function(g){return{key:g.key,label:mapName(g.key),value:g.value,color:'#54bd83'};}),max=Math.max(1,...gs.map(function(g){return g.value;}));document.getElementById('gatLocationChart').innerHTML=bars(gs,max,money);}
